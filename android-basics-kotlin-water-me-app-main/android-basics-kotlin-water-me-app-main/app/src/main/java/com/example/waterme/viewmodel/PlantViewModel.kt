...
class PlantViewModel(application: Application): ViewModel() {
    ...
    internal fun scheduleReminder(
        ...
        plantName.let {
            builder.putString(nameKey, plantName)
        }
        val build = builder.build()

        // TODO: Generate a OneTimeWorkRequest with the passed in duration, time unit, and data
        //  instance
        val request = PeriodicWorkRequestBuilder<WaterReminderWorker>(duration, unit)
        request.setInputData(build)
        val request1 = request.build()

        // TODO: Enqueue the request as a unique work request
        workManager.enqueueUniquePeriodicWork(plantName, ExistingPeriodicWorkPolicy.REPLACE, request1)
